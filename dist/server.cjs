...
var FALLBACK_BLOG_POSTS, DIRECT_WP_BASE, FALLBACK_FAQS, FALLBACK_REQUIREMENT_POSTS;
...
app.get("/api/wordpress/posts", async (req, res) => {
  try {
      ...
      if (catRes.ok) {
        ...
        if (Array.isArray(categories)) {
          const matchedCat = categories.find(
            (c) => c.id === 16 || c.name?.toLowerCase().includes("urgent vietnam visa blog new") || c.slug?.toLowerCase().includes("urgent-vietnam-visa")
          );
          if (matchedCat) {
    ...
    const postsUrl = categoryId ? `${wpBaseUrl}/wp-json/wp/v2/posts?categories=${categoryId}&per_page=100&_embed=true` : `${wpBaseUrl}/wp-json/wp/v2/posts?per_page=100&_embed=true`;
    ...
    if (postsRes.ok) {
      ...
      if (Array.isArray(wpPosts) && wpPosts.length > 0) {
        const formattedPosts = wpPosts.map((p) => {
          let featuredImage = "https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=crop&w=800&q=80";
          if (p._embedded && p._embedded["wp:featuredmedia"] && p._embedded["wp:featuredmedia"][0]) {
            featuredImage = p._embedded["wp:featuredmedia"][0].source_url || featuredImage;
          }
    ...
    return res.json({ success: true, posts: FALLBACK_BLOG_POSTS2, source: "fallback_curated" });
  } catch (err) {
    console.error("WordPress API Proxy Error:", err);
    const { FALLBACK_BLOG_POSTS: FALLBACK_BLOG_POSTS2 } = await Promise.resolve().then(() => (init_wordpressApi(), wordpressApi_exports));
    return res.json({ success: true, posts: FALLBACK_BLOG_POSTS2, source: "fallback_error" });
  }
});
  ...
  try {
    ...
    try {
      ...
      if (catRes.ok) {
        ...
        if (Array.isArray(categories)) {
          const matchedCat = categories.find(
            (c) => c.id === 71 || c.slug === "faq" || c.name?.toLowerCase() === "faq"
          );
          if (matchedCat) {
    ...
    if (faqsRes.ok) {
      ...
      if (Array.isArray(wpFaqs) && wpFaqs.length > 0) {
        const formattedFaqs = wpFaqs.map((p) => {
          let featuredImage = "";
          if (p._embedded && p._embedded["wp:featuredmedia"] && p._embedded["wp:featuredmedia"][0]) {
    ...
    return res.json({ success: true, faqs: FALLBACK_FAQS2, source: "fallback_curated" });
  } catch (err) {
    console.error("WordPress FAQ Proxy Error:", err);
    const { FALLBACK_FAQS: FALLBACK_FAQS2 } = await Promise.resolve().then(() => (init_wordpressApi(), wordpressApi_exports));
    return res.json({ success: true, faqs: FALLBACK_FAQS2, source: "fallback_error" });
  }
});
  ...
  try {
    ...
    try {
      ...
      if (catRes.ok) {
        ...
        if (Array.isArray(categories)) {
          const matchedCat = categories.find(
            (c) => c.slug?.toLowerCase().includes("visa-requirement") || c.slug?.toLowerCase().includes("requirement") || c.name?.toLowerCase().includes("visa requirement") || c.name?.toLowerCase().includes("requirement")
          );
          if (matchedCat) {
    ...
    const reqUrlBase = reqCategoryId ? `${wpBaseUrl}/wp-json/wp/v2/posts?categories=${reqCategoryId}&per_page=100&_embed=true` : `${wpBaseUrl}/wp-json/wp/v2/posts?per_page=100&_embed=true`;
    let rawWpPosts = [];
    let page = 1;
    let totalPages = 1;
    do {
    ...
    if (rawWpPosts.length > 0) {
      const formattedPosts = rawWpPosts.map((p) => {
        let featuredImage = "https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=crop&w=800&q=80";
        if (p._embedded && p._embedded["wp:featuredmedia"] && p._embedded["wp:featuredmedia"][0]) {
          featuredImage = p._embedded["wp:featuredmedia"][0].source_url || featuredImage;
        }
    ...
    return res.json({ success: true, posts: FALLBACK_REQUIREMENT_POSTS2, source: "fallback_curated" });
  } catch (err) {
    console.error("WordPress Requirements Proxy Error:", err);
    const { FALLBACK_REQUIREMENT_POSTS: FALLBACK_REQUIREMENT_POSTS2 } = await Promise.resolve().then(() => (init_wordpressApi(), wordpressApi_exports));
    return res.json({ success: true, posts: FALLBACK_REQUIREMENT_POSTS2, source: "fallback_error" });
  }
});
