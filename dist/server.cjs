...
app.get("/api/wordpress/requirements", async (req, res) => {
  try {
    ...
    const reqUrl = reqCategoryId ? `${wpBaseUrl}/wp-json/wp/v2/posts?categories=${reqCategoryId}&per_page=100&_embed=true` : `${wpBaseUrl}/wp-json/wp/v2/posts?per_page=100&_embed=true`;
