...
async function fetchWpPostBySlug(slug, categoryId) {
  const catQuery = categoryId !== void 0 ? `&category=${encodeURIComponent(categoryId)}` : "";
  try {
  ...
  try {
    const directCatQuery = categoryId !== void 0 ? `&categories=${encodeURIComponent(categoryId)}` : "";
    const directRes = await fetch(`${DIRECT_WP_BASE}/wp-json/wp/v2/posts?slug=${encodeURIComponent(slug)}${directCatQuery}&_embed=true`);
    if (directRes.ok) {
      const posts = await directRes.json();
      if (Array.isArray(posts) && posts.length > 0) {
        const p = posts[0];
        if (categoryId !== void 0 && Number(categoryId) === 70 && Array.isArray(p.categories) && !p.categories.includes(70)) {
          return null;
        }
        let featuredImage = "https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=crop&w=800&q=80";
        if (p._embedded && p._embedded["wp:featuredmedia"] && p._embedded["wp:featuredmedia"][0]) {
...
app.get("/api/wordpress/post-by-slug", async (req, res) => {
  ...
  const isRequirementPost = categoryParam === "70" || categoryParam === "requirements" || slug.startsWith("vietnam-visa-requirements") || slug.startsWith("vietnam-e-visa-for-");
